...
test_display_reservation(const std::string &root)
{
    ...
    {
        auto first = xmin::next::DisplaySocket::open(std::nullopt, root);
        if (!first || first.value().display() != 0 ||
            !exists(first.value().socket_path()) ||
            !exists(first.value().lock_path())) {
            return false;
        }
        auto duplicate = xmin::next::DisplaySocket::open(0, root);
        if (duplicate)
            return false;
        auto second = xmin::next::DisplaySocket::open(std::nullopt, root);
...
test_replacement_paths_are_preserved(const std::string &root)
{
    ...
    {
        auto display = xmin::next::DisplaySocket::open(12, root);
        if (!display)
            return false;
