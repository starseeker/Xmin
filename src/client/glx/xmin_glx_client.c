...
extern XVisualInfo *XGetVisualInfo(Display *, long, XVisualInfo *, int *) XMIN_WEAK;
extern Status XGetGeometry(Display *, XID, Window *, int *, int *,
                           unsigned int *, unsigned int *, unsigned int *,
                           unsigned int *) XMIN_WEAK;
extern GC XCreateGC(Display *, XID, unsigned long, void *) XMIN_WEAK;
extern int XFreeGC(Display *, GC) XMIN_WEAK;
extern XImage *XCreateImage(Display *, Visual *, unsigned int, int, int, char *,
                            unsigned int, unsigned int, int, int) XMIN_WEAK;
extern int XPutImage(Display *, XID, GC, XImage *, int, int, int, int,
                     unsigned int, unsigned int) XMIN_WEAK;
extern int XFlush(Display *) XMIN_WEAK;
extern int XSync(Display *, Bool) XMIN_WEAK;
extern int XFree(void *) XMIN_WEAK;
...
static pthread_once_t xmin_config_once = PTHREAD_ONCE_INIT;
static pthread_mutex_t xmin_drawable_mutex = PTHREAD_MUTEX_INITIALIZER;
static struct xmin_drawable *xmin_drawables;
static GLXDrawable xmin_next_drawable = 0x70000000UL;
static _Thread_local GLXContext xmin_current_context;
static _Thread_local GLXDrawable xmin_current_draw;
static _Thread_local GLXDrawable xmin_current_read;
static _Thread_local Display *xmin_current_display;
static _Thread_local unsigned int xmin_swap_interval;

#if XMIN_HAVE_QT_XCB
struct xmin_xcb_display {
    Display *token;
    xcb_connection_t *connection;
    struct xmin_xcb_display *next;
};

static pthread_mutex_t xmin_xcb_display_mutex = PTHREAD_MUTEX_INITIALIZER;
static struct xmin_xcb_display *xmin_xcb_displays;

static xcb_connection_t *
xmin_xcb_connection(Display *display)
{
    struct xmin_xcb_display *entry;
    xcb_connection_t *connection = NULL;

    pthread_mutex_lock(&xmin_xcb_display_mutex);
    for (entry = xmin_xcb_displays; entry != NULL; entry = entry->next) {
        if (entry->token == display) {
            connection = entry->connection;
            break;
        }
    }
    pthread_mutex_unlock(&xmin_xcb_display_mutex);
    return connection;
}

Display *
xminGlxCreateXcbDisplay(xcb_connection_t *connection)
{
    struct xmin_xcb_display *entry;
...
{
    struct xmin_xcb_display **link;

    if (display == NULL)
        return;
    pthread_mutex_lock(&xmin_xcb_display_mutex);
    for (link = &xmin_xcb_displays; *link != NULL; link = &(*link)->next) {
        if ((*link)->token == display) {
            struct xmin_xcb_display *removed = *link;
...
xmin_visual_info(Display *display, int screen)
{
    if (XGetVisualInfo != NULL && display != NULL) {
        ...
        template.class = XMIN_TRUE_COLOR;
    ...
    {
        XVisualInfo *info = calloc(1, sizeof(*info) + sizeof(Visual));
        Visual *visual;

        if (info == NULL)
            return NULL;
        visual = (Visual *) (info + 1);
        visual->class = XMIN_TRUE_COLOR;
...
{
    struct xmin_drawable *drawable = calloc(1, sizeof(*drawable));

    if (drawable == NULL)
        return 0;
    pthread_mutex_lock(&xmin_drawable_mutex);
    ...
    pthread_mutex_unlock(&xmin_drawable_mutex);
...
#if XMIN_HAVE_QT_XCB
    {
        xcb_connection_t *connection = xmin_xcb_connection(display);

        if (connection != NULL) {
            xcb_get_geometry_reply_t *reply = xcb_get_geometry_reply(
                connection, xcb_get_geometry(connection, (xcb_drawable_t) handle),
                NULL);
...
#if XMIN_HAVE_QT_XCB
    {
        ...
        if (connection != NULL) {
            xcb_get_geometry_reply_t *geometry = xcb_get_geometry_reply(
                connection, xcb_get_geometry(connection, (xcb_drawable_t) xid),
                NULL);
            ...
            free(geometry);
            image_size = (uint64_t) width * height * 4;
            if (width != storage_width || height != storage_height ||
                pixels == NULL || (depth != 24 && depth != 32) ||
                image_size > UINT32_MAX)
                return 0;
            xcb_gc = xcb_generate_id(connection);
            xcb_create_gc(connection, xcb_gc, (xcb_drawable_t) xid, 0, NULL);
            xcb_put_image(connection, XCB_IMAGE_FORMAT_Z_PIXMAP,
                          (xcb_drawable_t) xid, xcb_gc,
                          (uint16_t) width, (uint16_t) height,
                          0, 0, 0, (uint8_t) depth,
                          (uint32_t) image_size, pixels);
...
glXChooseVisual(Display *display, int screen, int *attributes)
{
    ...
    if (!rgba)
        return NULL;
    pthread_once(&xmin_config_once, xmin_init_configs);
               ...
               unsigned long mask)
{
    ...
    /* OSMesa's context embeds Mesa's GLcontext as its first member.  The
     * renderer's state copier implements the GL attribute-group semantics
     * required by glXCopyContext; shared objects are deliberately not part of
     * this operation.  GLX forbids a current destination, so leave it
     * untouched when that case can be detected in this thread. */
    if (source == NULL || destination == NULL || source == destination ||
        destination == xmin_current_context)
        return;
    _mesa_copy_context((const GLcontext *) source->osmesa,
                       (GLcontext *) destination->osmesa, (GLuint) mask);
}
...
glXGetFBConfigs(Display *display, int screen, int *count)
{
    ...
    (void) display;
    pthread_once(&xmin_config_once, xmin_init_configs);
    result = calloc(XMIN_CONFIG_COUNT, sizeof(*result));
    if (result == NULL) {
        if (count != NULL)
            *count = 0;
        return NULL;
